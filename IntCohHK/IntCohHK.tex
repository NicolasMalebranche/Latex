...
Denote $S\hilb{3}$ the Hilbert scheme of 3 points on a projective K3 surface. We identify $\Sym^2H^2(S\hilb{3},\IZ)$ with its image in $H^4(S\hilb{3},\IZ)$ under the cup product mapping. 
\begin{theorem}
...
If there exists a $0\neq v \in L$, such that $B(v,v)= 0$, the lattice is called indefinite. 
...
For all the author knews, the result for $n=3$ is new.
...
\item For $n=4$, $L$ is an odd lattice of rank 19298 and signature 2574.
\end{enumerate}
For $n$ even, $L$ is always odd. 
\end{proposition} 
\begin{proof}The numerical results come from an explicit calculation. For $n$ even, we always have the norm-1-vector given by Example \ref{example} (\ref{oddWitness}), so $L$ is odd.
\end{proof}
