...
\begin{proof} This is a simple combinatorial observation. The lower bound is witnessed by $x^{(\lfloor\frac{k}{2}\rfloor)}$ and the upper bound is witnessed by $\One^{(2^k)}$.
\end{proof}
\end{lemma}
...
\end{theorem}
\begin{corollary} \label{stabCor} Let $\lambda^0,\mu^0,\nu^0$ be partitions containing no Ones. Then the coefficients $c_i$ of the cup product in $H^\ast(S\hilb{n},\IZ)$
$$
...
\end{corollary}
\emph{Proof.} Assume $n\geq \sum \|\lambda^j\|, \sum \|\mu^j\| $. Set $Q_1:=  \kq_{\lambda^0}(1)\kq_{\lambda^{23}}(x)\prod_{1\leq j\leq 22}\kq_{\lambda^j}(\alpha_j),$ $ Q_2:= \kq_{\mu^0}(1)\kq_{\mu^{23}}(x)\prod_{1\leq j\leq 22}\kq_{\mu^j}(\alpha_j),$ $ Q_3:=  \kq_{\nu_i^0}(1)\kq_{\nu_i^{23}}(x)\prod_{1\leq j\leq 22}\kq_{\nu_i^j}(\alpha_j)$. Let $n_1,n_2,n_3$ be defined as above. Then:
$ \One^{\lambda^0}\alpha_1^{\lambda^1}\ldots\alpha_{22}^{\lambda^{22}}x^{\lambda^{23}} \smile 
\One^{\mu^0}\alpha_1^{\mu^1}\ldots\alpha_{22}^{\mu^{22}}x^{\mu^{23}}
= \frac{1}{(n-n_1)!\,z_{\lambda^0}}\kq_{n-n_1}(1)Q_1\vac\smile\frac{1}{(n-n_2)!\,z_{\mu^0}}\kq_{n-n_2}(1)Q_2\vac $. Thus the coefficient $c_i$ in the product expansion is some multiple of $\frac{(n-n_3)!}{(n-m)!}$ for a certain $n\leq n_1+n_2$. This is a polynomial of degree $m-n_3\leq n_1+n_2-n_3$.
...
\end{proposition}
\begin{proof} The last assertion for arbitrary $n$ follows from \ref{stabCor}. First, observe that if $\One^{\lambda^0}\alpha_1^{\lambda^1}\ldots\alpha_{22}^{\lambda^{22}}x^{\lambda^{23}}$ has cohomological degree $2k$, then $\frac{k}{2}\leq\sum \|\lambda^j\|\leq 2k$. The coefficients of the cup product martix are polynomials of degree at most $2+4-2 =4$ and it suffices to compute only a finite number of instances for $n$. It turns out, that the maximal degree is 1 and its cokernel is given as stated.
\end{proof}
\begin{remark}
