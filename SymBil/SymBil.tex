...
$$
\frac{\det \tilde{\G}}{\det\G} = \prod_{|\alpha|=k}a^{\alpha_d} = \prod_{i=0}^k\ \prod_{|\alpha '|=k-i} a^i  =\prod_{i=0}^ka^{i\binom{d-1+k-i}{d-1}} = a^{\binom{d+k}{d+1}}
$$
For the last equality we have used Lemma \ref{binomialLemma}. Now we can reduce the proof to the case when $G$ is the identity matrix. Then the statement is established by Corollary \ref{thetaCor}. By an orthogonal transformation we may assume that $G= \text{diag}\left(a_0,\ldots,a_d\right)$ is a diagonal matrix. Then we apply successively coordinate transformations that map $x_i$ to $\frac{x_i}{\sqrt{a_i}}$. We get a factor $(a_0\ldots a_d)^{\binom{d+k}{d+1}} = \det G^{\binom{d+k}{d+1}}$.
\end{proof}
...
$\binom{z}{k} \coloneqq \frac{z(z-1)\ldots(z-k+1)}{k!}$. For negative $k$ we set $\binom{z}{k}\coloneqq 0$. 
\end{definition}
The binomial coefficient satisfies the difference equation: $\binom{z+1}{k}-\binom{z}{k} = \binom{z}{k-1}$.
The rank of $\Sym^k K^{d+1}$ equals $\binom{d+k}{k}=\binom{d+k}{d}$.
\begin{lemma} \label{binomialLemma}
The following formula holds true for integers $0\leq j\leq k\leq n$:
$$ \sum_{m=0}^n\binom{m}{j}\binom{n-m}{k-j} = \binom{n+1}{k+1}
$$
\end{lemma}
\begin{proof}This is a standard identity in the calculus of binomial coefficients.
\end{proof}

\begin{lemma}
 The following identities hold true for integers $d,k,m\geq 0$:
...
\end{lemma}
\begin{proof}
This is easily proven by induction over $k$.
\end{proof}
...
\end{proof}
\begin{corollary}\label{thetaCor} Let $D(d,k):=\det(\bra x^\alpha ,x^\beta \ket)_{|\alpha|,|\beta|=k} $ be the determinant of the Gram matrix of $\bra\ ,\;\ket$. Then:
$$
D(d,k) = \theta_{d,k}
$$
where $\theta_{d,k}$ is defined as in Equation (\ref{thetaDef}).
\end{corollary}
\begin{proof} We use a double induction over $k$ and $d$.
From the above theorem it is clear that $D(d,k)=\prod_{|\alpha | =k}\bra h_\alpha ,h_\alpha \ket$. Now observe that the equality holds for $d\leq 0$. Since $\{ |\alpha| = k \} = \bigcup_{j=0}^k  \{ |\alpha'| = j \} \times \{ k-j \}  $, we have from Equation (\ref{hth1}):
$$
...
Furthermore, $R(d,k)$ fulfils the recurrence
\begin{align*}
 \frac{R(d,k+1)}{R(d,k)} &= \frac{(2k\!+\!d\!+\!2)!!^{\binom{k+d}{d-1}}\,(2k\!+\!d\!+\!1)^{\binom{k+d+1}{d}}\,(2k\!+\!d\!+\!1)!!^{\binom{k+d}{d-1}}}{(2k\!+\!d\!+\!2)!^{\binom{k+d}{d-1}} \prod_{j=0}^k(j\!+\!k\!+\!d\!+\!1)^{\binom{j+d-1}{d-1}}} \prod_{i=1}^{k+1} i^{\binom{k-i+d}{d-1}}\\
&= \frac{(2k+d+1)^{\binom{k+d}{d}}}{\prod_{j=0}^k(j\!+\!k\!+\!d\!+\!1)^{\binom{j+d-1}{d-1}}}\prod_{i=1}^{k+1} i^{\binom{k-i+d}{d-1}}.
\end{align*}
We will now show the principal inductive step: $\frac{D(d,k+1)}{D(d,k)D(d-1,k+1)}=\frac{\theta_{d,k+1}}{\theta_{d,k}\theta_{d-1,k+1}}$. The left hand side, clearly equals $\frac{R(d,k+1)}{R(d,k+1)} $.

%$$
%\frac{\theta_{d,k}}{\prod_{j=0}^k \theta_{d-1,j} } = \rho_{d,k}\prod_{i=1}^k i^{\binom{k-i+d}{d}} 
%$$ where 
$$
\frac{\theta_{d,k}}{\prod_{j=0}^k \theta_{d-1,j} } := \left\{
 \begin{array}{*2{l}p{5cm}}
 \displaystyle \prod_{j=0}^k \prod_{i=1}^{j+\frac{d}{2}-1} i^{\binom{j-2i+d-1}{d-1}}\prod_{\substack{i=1 \\ i\ \text{odd}\\\ }}^{2k+d-1}i^{\binom{k-i+d}{d}} &\text{if }d\text{ is even}, \\
 \displaystyle\prod_{i=1}^k i^{\binom{k-i+d}{d}d}\prod_{i=1}^{k+\frac{d-1}{2}} i^{\binom{k-i+d}{d} - \binom{k-2i+d}{d}} &\text{if }d\text{ is odd}.
\end{array}
\right.
$$
