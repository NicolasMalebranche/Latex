...
\keywords{Symmetric Bilinear Forms on Symmetric Powers, Orthogonal Polynomials in Several Variables, Homogeneous Orthogonal Polynomials, Gegenbauer Polynomials, Hermite Polynomials, Spherical Harmonics, Hankel matrices, Hyperk\"ahler Manifolds, Beauville--Bogomolov Form, Beauville--Fujiki relation}
...
There are two topics that come close, see Remark \ref{hermite}. The first one is the theory of Hermite polynomials. The second is spherical harmonics, which imposes an unwanted restriction though, namely the condition for the polynomials being harmonic.
...
Choosing a basis $(x_i)_i$ of $L$, we may express $\discr L$ as the determinant of the so--called Gram matrix $G$ of $\left<\ ,\;\right>$, which is defined by $G_{ij}\coloneqq \left<x_i,x_j\right>$. 
...
We stress that this equivalency really depends on the homogeneity. Denote $\bra f ,g\ket_{\R^{d+1}} \coloneqq \bra f,g\ket$ and $\bra f ,g\ket_{\S^{d}} \coloneqq \int_{\S^{d}} f g$ for a moment and let us look at what happens if we drop the homogeneity constraint. Since $c_{d,k}$ depends on $k$, $\bra\ ,\;\ket_{\R^{d+1}}$ and $\bra\ ,\;\ket_{\S^d}$ aren't equivalent anymore:
\begin{itemize}
\item A basis of $\R[x_0,\ldots,x_d]$, consisting of $\bra f ,g\ket_{\R^{d+1}}$-orthogonal polynomials is given by products $H_{\alpha_0}\!(x_0)\ldots H_{\alpha_d}\!(x_d)$ of Hermite polynomials in one variable, see also \cite[Sect.~2.3.4]{Dunkl}.
\item The form $\bra\ ,\;\ket_{\S^d}$ becomes degenerate on $\R[x_0,\ldots,x_d]$, since integration on the sphere can't distinguish between $1$ and the square radius $\|x\|^2$. Ignoring thus any radial components, a basis of $\R[x_0,\ldots,x_d]/(\|x\|^2-1)$ consisting of $\bra\ ,\;\ket_{\S^d}$-orthogonal polynomials is given by spherical harmonics, see also \cite[Sect.~2.2]{Dunkl}.
\end{itemize}
\end{remark}
...
\end{definition}
\begin{remark}
As Yuan Xu pointed out, this can be written in terms of the hypergeometric function, namely $\p{n}{m}(x) = x^n\vphantom{}_2F_1\!\left( \genfrac{}{}{0pt}{}{-\frac{n}{2},\,\frac{1-n}{2}}{\frac{m}{2}-n+1} ; -\frac{1}{x^2}\right)$. Too see this, first change summation from $j$ to $n-j$, so that the sum is over $j\! =$ even, then set $j\! =\!2i$ and rewrite the sum in the notation of the rising Pochhammer symbol $(a)_n = a (a\!+\!1)\ldots(a\!+\!n\!-\!1)$. Comparing this formula with \cite[Prop.~1.4.11]{Dunkl}, it follows that the $\p{n}{m}$ are in fact a variant of the Gegenbauer polynomials $C_n^\lambda$. More precisely, $\p{n}{m}(x)$ is a multiple of $C^{-\frac{m}{2}}_n\!\!\left(\sqrt{-1}\,x\right)$, where the factor is chosen such that the polynomial becomes monic.
\end{remark}
...
\end{proof}
\begin{theorem}\label{thetaCor} Let the biliniear form $\bra\ ,\;\ket$ on $\R[x_0,\ldots x_d]_k$ be defined as in the beginning of Section \ref{polynomialSection}. Let $D(d,k):=\det\limits_{|\alpha|,|\beta|=k}\bra x^\alpha ,x^\beta \ket $ be the determinant of the Gram matrix of $\bra\ ,\;\ket$. Then:
$$
D(d,k) = \theta_{d,k}
$$
...
Let $S^{[k]}$ for $k\geq 2$ be the Hilbert scheme of $k$ points on a K3 surface $S$, let $K^{[[k]]}$ be the generalized Kummer variety of a torus and let $OG_6$ and $OG_{10}$ be the 6-- resp. 10--dimensional O'Grady manifold. Then we have:
...
\item $2k+5$, if $X$ is deformation equivalent to $K^{[[k]]}$, 
