...
\keywords{Orthogonal Polynomials in Several Variables, Hankel matrix, Homogeneous Polynomials, Hermite Polynomials, Hyperk\"ahler Manifolds, Beauville-Bogomolov Form, Fujiki relation}
...
For later reference, please note that the number of partitions of $\{1,\ldots,2k\}$ into pairs is given by the double factorial $(2k-1)!! = (2k-1)(2k-3)(2k-5)\ldots$.
\begin{example}
...
Let $X$ be a compact Hyperk\"ahler manifold of complex dimension $2k$. On the second cohomology $H^2(X,\Z)$ there is an integral quadratic form, called the Beauville--Bogomolov form $q_X$, which can be computed by an integration over some cup--product power:
\begin{equation}
\int_X \alpha ^{2k} = c_X q_X(\alpha)^k
\end{equation}
See \cite[Chap. 23]{Huybrechts} or \cite[Subsection~2.3]{OGrady} for reference.
