...
\end{corollary}
\begin{proof} This is a technical computation.
...
hence 
\begin{align*}
\frac{D(d,k)}{\prod_{j=0}^k D(d\!-\!1,j) } \;&=\; \prod_{j=0}^k \left[(k-j)!\,\frac{(2j+d)!!\,(2k+d-1)!!}{(j+k+d)!} \right]^{\binom{j+d-1}{d-1}} \\
&=\; \prod_{i=1}^k i^{\binom{k-i+d}{d}} \underbrace{\frac{\displaystyle\prod_{i=k+d+1}^{2k+d} i^{\binom{i-k-1}{d} }}{\displaystyle \prod_{\substack{i=d+1 \\ i+d\text{ even}}}^{2k+d} i^{\binom{\frac{i+d}{2}-1}{d} }    } }_{=:R(d,k)}
\end{align*}
